...
const addNewBook = async (e) => {
    ...
    if (isEditing === false) {
        await fetch("http://localhost:1234/db-api-utn/books", {
        method: "POST",
        ...
        const response = await fetch("http://localhost:1234/db-api-utn/books/" + idToEdit, {
        method: "PATCH",
        body: JSON.stringify(bookData),
        headers: { "Content-Type": "application/json" }
        })
...
const deleteBook = async (id) => {
    ...
    if (confirm("Esta seguro de que quieres borrar este libro?")) {
        const response = await fetch("http://localhost:1235/db-api-utn/books/" + id, {
        method: "DELETE"
        })
        fetchingBooks()
    }
